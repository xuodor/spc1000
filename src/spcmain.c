...
int LoadImageFile(void)
{
	...
	ofn.lpstrFile[0] = '\0';
	ofn.nMaxFile = 260;
	ofn.lpstrFilter = "Image\0*.SAV\0All\0*.*\0";
	ofn.nFilterIndex = 1;
...
int OpenTapeFile(void)
{
	...
	if (GetOpenFileName(&ofn)==TRUE)
	{
		...
		printf("Reading %s\n", szFile);
#endif
		if ((spc.IO.cas.rfp = fopen(szFile,"rb")) == NULL)
		{
			printf("File(%s) (read) open error.\n", szFile);
			return -1;
		}
...
int SaveAsTapeFile(void)
{
	...
	strncpy(szFile, &(spc.RAM[0x1397]),16);
	szFile[16] = '\0';
	strcat(szFile, ".TAP");
	...
	if (GetSaveFileName(&ofn)==TRUE)
	{
		...
		printf("Writing %s\n", szFile);
#endif
		if ((spc.IO.cas.wfp = fopen(szFile,"wb")) == NULL)
		{
			printf("File(%s) (write) open error.\n", szFile);
			return -1;
		}
...
int CasRead(Cassette *cas)
{
	...
	curTime = (spc.tick * 125)+((4000-spc.Z80R.ICount) >> 5) - spc.IO.cas.startTime;
	bitTime = (cas->rdVal == 0) ? 56: (cas->rdVal == 1)? 112 : 0;

	while (curTime >= (cas->cnt0 * 56 + cas->cnt1 * 112 + bitTime))
	{
		if (cas->rdVal == 0) cas->cnt0++;
		if (cas->rdVal == 1) cas->cnt1++;
...
void BuildKeyHashTab(void)
{
	...
	{
		KeyHashTab[i].keys 
			= malloc(sizeof(TKeyMap) * hashPos[i]);
	}
...
byte InZ80(register word Port)
{
	...
	else if ((Port & 0xFFFE) == 0x4000) // PSG
	{
		if (Port & 0x01) // Data
		{
			...
			else return RdData8910(&spc.IO.ay8910);
		}
	}
	return 0;
}
...
int main(int argc, char* argv[])
{
	...
	while (1)	// Main emulation loop
	{
		...
		if (R->ICount <= 0)	// 1 msec counter expired
		{
			...
			if (spc.intrTime < 0)	// 1/60 sec interrupt timer expired
			{
				spc.intrTime += INTR_PERIOD;	// re-init interrupt timer

				if (spc.refrTimer <= 0)			// check refresh timer
				{
					if (spc.IO.GMODE & 0x08)
						UpdateMC6847Gr(MC6847_UPDATEALL);
					else
				...
				{
					if (spc.refrSet != spcConfig.frameSkip)	// restore refresh timer start value
					{
			...
			if (simul.curTick-simul.prevTick > 0)	// elapsed >= 1msec?
			{
			...
			while (!turboState && (simul.curTick < spc.tick)) // Simulation is faster than real
			{
