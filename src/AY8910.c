...
void Reset8910(register AY8910 *D, int First) {
  static byte RegInit[16] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0xFD,
...
/*************************************************************/
void WrCtrl8910(AY8910 *D, byte V) { D->Latch = V & 0x0F; }

/** WrData8910() *********************************************/
/** Write a value V to the PSG Data Port.                   **/
/*************************************************************/
void WrData8910(AY8910 *D, byte V) { Write8910(D, D->Latch, V); }

/** RdData8910() *********************************************/
/** Read a value from the PSG Data Port.                    **/
/*************************************************************/
byte RdData8910(AY8910 *D) { return (D->R[D->Latch]); }
...
void Write8910(register AY8910 *D, register byte R, register byte V) {
    ...
    for (J = 0; R && (J < AY8910_CHANNELS); J++, R >>= 1, V >>= 1)
      if (R & 1) {
        if (V & 1) {
          ...
          I = J < 3 ? (((int)(D->R[J * 2 + 1] & 0x0F) << 8) + D->R[J * 2])
                    : (D->R[6] & 0x1F);
  ...
  case 10:
    ...
    // V&0x10? ((D->R[13]&0x04)? 0:255):255*(V&0x0F)/15; // ionique, for inc
    // envelope
    /* Start/stop envelope */
  ...
  case 13: // envelope shape/cycle control (ionique. originally the same as
           // 14,15)
    /* Write value */
    D->R[R] = V;
    for (I = 8; I <= 10; I++)
...
#define SndPeekQueue()                                                         \
  ((SndQ.front == SndQ.rear) ? -1 : SndQ.qentry[SndQ.rear].time)
